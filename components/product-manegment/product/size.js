...
function Size() {
  ...
  const {
    ...
    process.env.NEXT_PUBLIC_HOST_API +
      process.env.NEXT_PUBLIC_DELETE_PRODUCT_SIZE,
    "post",
    {},
    false
  );
  ...
  useEffect(() => {
    if (deleteData?.status === true) {
      //refresh the table ....
      message.success("Size has been deleted succeeefully !");
      executeFetch();
      ...
      Modal.info({
        ...
        content: (
          <p>
            Some error happend while trying to delete this size. Please try
            again later.
          </p>
        ),
  ...
  const handleDeleteItem = (id, name) => {
    Modal.confirm({
      title: "Are you sure about delete this size ",
      content: <div>this size will be deleted forever.....!</div>,
      onOk() {
  ...
  const handleEditItem = (id) => {
    dispatch(
      openModal(constants.modalType_AddSize, executeFetch, { id }, true)
    );
  };
  ...
  return (
    <Main>
      <Button
        onClick={() =>
          dispatch(openModal(constants.modalType_AddSize, executeFetch))
        }
      >
        Add New Size
      </Button>
