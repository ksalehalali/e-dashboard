...
function HomePage({ allCookies }) {
  ...
  useEffect(() => {
    const getData = async () => {
      const { data: TotalCategory } = await axios.get(
        "https://dashcommerce.click68.com/api/totalCategory",
        {
          headers: {
  ...
  const config = {
    data: [TotalPaymentDay, TotalPaymentMonth, TotalPaymentYear, TotalPayment],
    padding: "auto",
    seriesField: "name",
    xField: "type",
    yField: "value",
      ...
      <Row gutter={[32, 32]}>
        ...
        <Col span={24}>
          ...
          <Form form={form} layout="vertical" onFinish={onFinish}>
            <Row gutter={24}>
              <Col span={4}>
                <Form.Item name="from" label="select start date">
